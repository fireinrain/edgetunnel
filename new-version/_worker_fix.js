...
// 将kv数据库绑定到  变量 USER_IDS
let id = atob('ZWM4NzJkOGYtNzJiMC00YTA0LWI2MTItMDMyN2U4NWUxOGVk');
// let id = 'ec872d8f-72b0-4a04-b612-0327e85e18ed'
...
// let paddr = one of paddrs (random)
...
// let p64DnUrl = 'https://1.1.1.1/dns-query'

let p64Prefix = atob('MjYwMjpmYzU5OmIwOjY0Ojo=');
// let p64Prefix = '2602:fc59:b0:64::'
...
// let durl = 'https://sky.rethinkdns.com/1:-Pf____9_8A_AMAAgE8kMABVDDmKOHTAKg='
...
// let ytName = 'https://youtube.com/@am_clubs?sub_confirmation=1'

let tgName = atob('aHR0cHM6Ly90Lm1lL2FtX2NsdWJz');
// let tgName = 'https://t.me/am_clubs'

let ghName = atob('aHR0cHM6Ly9naXRodWIuY29tL2FtY2x1YnMvYW0tY2YtdHVubmVs');
// let ghName = 'https://github.com/amclubs/am-cf-tunnel'

let bName = atob('aHR0cHM6Ly9hbWNsdWJzcy5jb20=');
// let bName = 'https://amclubss.com'
        ...
        try {
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                if (prType === xorDe(dataTypeTr, 'datatype')) {
                    return await websvcExecutorTr(request, env);
                }
                return await websvcExecutor(request,env);
            }
            switch (url.pathname.toLowerCase()) {
                case '/': {
                    return await indexPage(request, env);
                }
                default: {
                    //remove / from pathname
                    const uuidStr = url.pathname.substring(1);
                    //check if a valid uuid
                    if (!isValidUserId(uuidStr)) {
                        return new Response('You are not a valid user!', {status: 404});
                    }
                    //check if user have a valid  subscription after sub expiration within 12days
                    //you need bind a kv namespace to this worker, see cloudflare dash
                    const validUserExpiredTime = await checkUserIdValid(request,uuidStr,env);
                    if (validUserExpiredTime || uuidStr === id) {
                        return new Response('You are valid for subscription, just enjoy it :)', {status: 200})
                    }else {
                        return new Response('You dont have permission to use,due to subscription expired for more than 12 days.' +
                            '\ngo to source site: https://cloud.paimon.eu to subscribe.', {status: 401});
                    }
...
async function websvcExecutor(request,env) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            if (realUserid !== id) {
                let validUser = await checkUserIdValid(request,realUserid,env);
                if (!validUser) {
...
async function websvcExecutorTr(request,env) {
    const webSocketPair = new WebSocketPair();
    const [client, webSocket] = Object.values(webSocketPair);
    webSocket.accept();
...
async function handleTPOut(remoteS, addressRemote, portRemote, rawClientData, pipe, channelResponseHeader, log, addressType) {
    ...
    async function connectAndWrite(address, port, socks = false) {
        const tcpS = socks ? await serviceCall(addressType, address, port, log) : connect({
            hostname: address,
            port: port,
            servername: addressRemote
        });
        remoteS.value = tcpS;
...
function handleRequestHeader(channelBuffer, id, env) {
    ...
    // if (stringify(new Uint8Array(channelBuffer.slice(1, 17))) === userID) {
    //     isValidUser = true;
    // }
    // if (!isValidUser) {
    //     return {
    ...
    //     };
    // }
    //默认都是valid user，但是需要再外面进行检测
    const newUserId = stringify(new Uint8Array(channelBuffer.slice(1, 17)));
...
//-------------checkUserIdValid-------------
//需要将KV存储绑定到CF_USER_IDS
async function checkUserIdValid(request, userId, env) {
    return await env.USER_IDS.get(userId);
...
async function indexPage(request, env) {
    const html = `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>CF Worker API</title>
  <style>
    :root {
      --bg: #0f172a;
      --card: #020617;
...
<body>
  <div class="card">
    ...
    <p>服务已成功运行。</p>
    <p>这是一个基于 Cloudflare Workers 的轻量 API 网关。</p>
